      ...
      {cartItems.length
        ? cartItems.map((item) => (
            <div key={item.id} className="relative">
              <div
                className={`absolute right-0 bottom-2 p-2 bg-powder-pink text-sm md:text-md ${hidden ? 'hidden' : ''}`}
              ...
              <div className="flex gap-3">
                ...
                <div className="shrink mr-[10px]">
                  ...
                  <div className="flex justify-between mt-[10px]">
                    {item.discount ? (
                      <div>
                        <span className="text-powder-red text-md">{`${Math.round(item.price - (item.price * item.discount) / 100) * (item.quantity || 1)}.00 грн`}</span>{' '}
                        <span className="line-through text-xs">{`${item.price * (item.quantity || 1)}.00 грн`}</span>
                      </div>
                    ) : (
